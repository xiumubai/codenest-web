...
// 模拟标签问题数据
const getQuestionsByTag = (tag: string) => [
  {
    id: 1,
...
export function TagQuestionsList({ tag }: { tag: string }) {
  const [sortBy, setSortBy] = useState("newest")
  const [currentPage, setCurrentPage] = useState(1)
  const questionsPerPage = 5

  // 获取与标签相关的问题
  const tagQuestions = getQuestionsByTag(tag)

  // 根据排序方式对问题进行排序
  const sortedQuestions = [...tagQuestions].sort((a, b) => {
    if (sortBy === "newest") {
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div className="text-sm text-muted-foreground">共 {tagQuestions.length} 个问题</div>
          ...
          >
            ...
            <div className="mb-2 flex flex-wrap gap-2">
              {question.tags.map((questionTag) => (
                <Link key={questionTag} href={`/tags/${encodeURIComponent(questionTag)}`}>
                  <Badge
                    variant="outline"
                    className={`text-xs font-normal ${
                      questionTag === tag ? "bg-primary/10 border-primary/20 text-primary" : ""
                    }`}
                  >
                    {questionTag}
                  </Badge>
